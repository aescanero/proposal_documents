#!/usr/bin/env python3
"""Genera 09-bloques-presentacion.svg (1920x1080, orientado a presentación).

Diagrama dibujado a mano, sin Mermaid: la fuente es este script. Para cambiar
el SVG se edita aquí y se ejecuta `python3 09-bloques-presentacion.py`.
Mismo estilo que sonarqube-qa/diagrams/20-bloques-presentacion.py.
"""
import os
...
o.append(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}" role="img" aria-label="Arquetipo keycloak: bloques y stacks">')
...
caps=[("cluster","GKE Standard · 2 zonas"),("policy","Gatekeeper · PSS restricted"),("ingress","Envoy Gateway · Gateway API"),
      ("certs","cert-manager · CA interna"),("secrets","ESO + Secret Manager"),("monitoring","Prometheus · Grafana · Loki"),
      ("database-platform","sin enlazar → Cloud SQL propio")]
y=LY+92
for c,d in caps:
...
cols=[('fund',"1 · Fundación"),('prot',"2 · Red y operador"),('app',"3 · Servidor"),('ops',"4 · Realm y operación")]
...
stack(xs[0],by,colw,bh,'fund',1,"iam","Namespace e identidades",["namespace keycloak","PSS restricted","KSA: keycloak · eso · config"])
stack(xs[0],by+bh+bg,colw,bh,'fund',2,"secrets","Secretos por referencia",["BD · admin · config-cli","clave de firma · Entra","ESO → Secrets de K8s"])
stack(xs[0],by+2*(bh+bg),colw,bh,'fund',3,"data","Cloud SQL dedicado",["qa-keycloak-main-g1","IP privada · PITR 7 días","gen_data.tm.hcl compartido"])
stack(xs[1],by,colw,bh,'prot',4,"firewall","Red cerrada por defecto",["default-deny entrada y salida","JGroups entre pods","Entra por FQDN vía NAT"])
stack(xs[1],by+bh+bg,colw,bh,'prot',5,"operator","Keycloak Operator",["solo el namespace keycloak","CRDs en templates/","resource-policy: keep"])
nx,ny=xs[1],by+2*(bh+bg)
rect(nx,ny,colw,bh,'#fffaf3',pal['prot'][1],rx=12,sw=1.5,dash="6 5")
text(nx+16,ny+34,"Ciclo de arranque (R22)",16,700,pal['prot'][1])
for i,l in enumerate(["Su ruta no lleva","SecurityPolicy, y los","consumidores llegan por","el Service interno."]):
...
text(ax+16,ay+76,"CR Keycloak · 2 réplicas",15,600)
text(ax+16,ay+98,"imagen optimizada · por digest",14,400,SUB)
pods=[("Pod keycloak-0","zona a · 2 GiB"),("Pod keycloak-1","zona b · 2 GiB")]
yy=ay+118
for n,d in pods:
...
text(ax+30,yy+26,"sidecar Auth Proxy",15,700,TXT); text(ax+30,yy+48,"unsupported.podTemplate",13,400,SUB,font=MONO); yy+=72
text(ax+16,yy+14,"HTTPS 8443 · gestión 9000",14,600,fg)
yy+=34
...
# panel derecho: origen y consumidores
RX,RY,RW,RH=1570,160,310,670
rect(RX,RY,RW,RH,pal['out'][0],'#a5f3fc',rx=18,sw=1.5)
text(RX+24,RY+44,"Provee oidc-idp 4.2.0",22,700)
text(RX+24,RY+70,"trait saml-idp · tenant: ConfigMap",15,400,SUB,italic=True)
items=[('up',"Entra ID (origen)","MFA · acceso condicional","app roles → claim roles"),
       ('out',"SonarQube","SAML · grupos team-*","ConfigMap client-sonarqube-*"),
       ('out',"Grafana","OIDC confidencial","secreto del propio consumidor"),
       ('out',"Apps en el Gateway","SecurityPolicy OIDC","backendRefs al Service")]
y=RY+92
...
ctrl=[("assert","ruta sin SecurityPolicy · paths"),("G1 conftest","redirectUris ⊆ claims · backendRefs"),("Gatekeeper","forma del ConfigMap de cliente"),
      ("G3 plan.json","sin secretos en el estado"),("Checkov","Cloud SQL privado y protegido")]
x=60; cw=364
for i,(a,b) in enumerate(ctrl):
    ...
    x+=cw
text(W-60,H-20,"docs/es/proposals/keycloak-qa/README.md",13,400,'#9ca3af','end',font=MONO)
o.append('</svg>')
