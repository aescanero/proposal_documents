...
o.append(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}" role="img" aria-label="Arquetipo secrets-eso-gsm: bloques">')
...
caps=[("cluster","GKE · Workload Identity",False),("policy","Gatekeeper · reglas propias",False),("certs","cert-manager · CA interna",False),
      ("Secret Manager","backend en GCP (AM §14.2)",True),("monitoring","NO se requiere: ciclo (§9.1)",True)]
y=LY+92
...
cols=[('fund',"1 · Instalación"),('prot',"2 · Seguridad"),('app',"3 · Contrato"),('ops',"4 · Operación")]
...
text(nx+16,ny+34,"Si cae el controlador…",16,700,pal['prot'][1])
for i,l in enumerate(["puede leer todo Secret:","namespace cerrado, sin","exec, y auditoría por","principal de tenant."]):
...
items=[("SonarQube","5 secretos","KSA eso-sonarqube"),("Keycloak","5 secretos + clientes OIDC","KSA eso-keycloak"),
       ("Monitorización","Alertmanager · Grafana","KSA eso-monitoring"),("Stack data (Cloud SQL)","escribe la versión de -db","no lee: solo escribe")]
y=RY+92
for t,a,b in items:
...
ctrl=[("assert","CRDs keep · webhook en 10250"),("Gatekeeper","kinds prohibidos · forma"),("G1 conftest","IAM por secreto · trait eso"),
      ("G3 plan.json","sin secretos en el estado"),("audit log","lectura fuera de la lista")]
x=60; cw=364
for i,(a,b) in enumerate(ctrl):
    ...
    x+=cw
text(W-60,H-20,"docs/es/proposals/external-secrets-qa/README.md",13,400,'#9ca3af','end',font=MONO)
o.append('</svg>')
