#!/usr/bin/env python3
"""Genera 20-bloques-presentacion.svg (1920x1080, orientado a presentación).

Diagrama dibujado a mano, sin Mermaid: la fuente es este script. Para cambiar
el SVG se edita aquí y se ejecuta `python3 20-bloques-presentacion.py`.
"""
...
o.append(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}" role="img" aria-label="Arquetipo sonarqube: bloques y stacks">')
...
cols=[('fund',"1 · Fundación"),('prot',"2 · Protección e identidad"),('app',"3 · Aplicación"),('ops',"4 · Publicación y operación")]
...
stack(xs[0],by,colw,bh,'fund',1,"iam","Namespace e identidades",["namespace sonarqube","PSS restricted","KSA: sonarqube · eso · db"])
...
text(nx+16,ny+34,"Sin SecurityPolicy OIDC",16,700,pal['prot'][1])
for i,l in enumerate(["La ruta la usan personas y","el scanner con token propio:","la autenticación la hace","SonarQube (SAML + tokens)."]):
...
text(ax+16,ay+98,"imagen propia · por digest",14,400,SUB)
...
text(ax+16,yy+14,"12 GiB · sin límite de CPU",14,600,fg)
yy+=34
...
ctrl=[("assert","digest · heaps · sin privilegiados"),("G1 conftest","input ↔ after · IAM por recurso"),("G2 Checkov","bucket privado y uniforme"),
      ("G3 plan.json","sin secretos en el estado"),("Gatekeeper","PSS · etiquetas · registro")]
x=60; cw=364
